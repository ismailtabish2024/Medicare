...
const API_BASE = import.meta.env.VITE_API_URL || "https://medicare-backend-febv.onrender.com";
...
export default function AppointmentsPage() {
  ...
  const loadDoctorAppointments = useCallback(async () => {
    ...
    console.log("Outgoing headers for /api/appointments:", headers);

    try {
      const resp = await API.get("/api/appointments", { headers });
      console.log("Response from /api/appointments:", resp?.data);
      ...
      console.error(
        "Error calling /api/appointments:",
      ...
      if (user?.id) {
        try {
          ...
          const debugResp = await API.get(
            `/api/appointments?createdBy=${user.id}`,
            { headers },
          );
  ...
  const loadServiceAppointments = useCallback(async () => {
    ...
    const headers = token ? { Authorization: `Bearer ${token}` } : {};
    console.log("Outgoing headers for /api/service-appointments:", headers);

    try {
      const resp = await API.get("/api/service-appointments", { headers });
      console.log("Response from /api/service-appointments:", resp?.data);
      ...
      console.error(
        "Error calling /api/service-appointments:",
        err?.response?.data || err.message || err,
      );
      ...
      if (user?.id) {
        try {
          console.log("Attempting debug request with ?createdBy=", user.id);
          const debugResp = await API.get(
            `/api/service-appointments?createdBy=${user.id}`,
            { headers },
          );
