...
const AddPage = () => {
  ...
  if (form.imagePreview && form.imageFile) {
    try {
      ...
      console.error("Error revoking image preview:", err);
      showToast("error", "Failed to clear previous image preview");
    }
  }
  ...
  try {
    ...
    showToast("error", "Failed to load selected image");
  }
}
 ...
 function removeImage() {
  if (form.imagePreview && form.imageFile) {
    try {
      ...
      showToast("error", "Failed to remove image preview");
    }
  }
  ...
  try {
    ...
    console.error("Error resetting image input:", err);
    ...
    try {
      ...
      if (form.imagePreview && form.imageFile) {
        try {
          ...
          // Silently ignore if revoke fails
        }
      }
      ...
      if (fileInputRef.current) {
        try {
          ...
          // Silently ignore if ref is unavailable
        }
      }
