...
export const getAppointments = async (req, res) => {
  try {
    ...
    console.error("getAppointments error:", err.message, err.stack);
...
// get appointments by patent
export const getAppointmentsByPatient = async (req, res) => {
  try {
    const queryCreatedBy = req.query.createdBy || null;
    const clerkUserId = req.auth?.userId || null; // fixed typo from ckerkUserId
    const resolvedCreatedBy = queryCreatedBy || clerkUserId || null;

    console.log("resolvedClerkUserId", resolvedCreatedBy);
    ...
    console.error("getAppointmentsby patent error:", err);
    return res
      .status(500)
...
export const createAppointment = async (req, res) => {
  try {
    ...
    const existingBooking = await Appointment.findOne({
      doctorId,
      createdBy: clerkUserId, // fixed field name
      date: String(date),
      time: String(time),
      status: { $ne: "canceled" },
    }).lean();
    ...
    try {
      ...
      console.warn("doctor lokup feild", err?.message || err);
    }
...
export const confirmPayment = async (req, res) => {
  try {
    ...
    if (!stripe)
      return res.status(500).json({
        success: false,
        message: "please configure stripe",
      });
    ...
    try {
      ...
      console.error("stripe tetrivr session error ", err);
      return res.status(404).json({
        success: false,
...
export const updateAppointment = async (req, res) => {
  try {
    ...
    const updated = await Appointment.findByIdAndUpdate(
      ...
      path: "doctorId",
      select: "name specialization imageUrl",
    });
...
export const cancelAppointment = async (req, res) => {
  try {
    ...
    appt.status = "Canceled"; // corrected spelling to match enum
    await appt.save();
    return res.json({
      success: true,
      appointment: appt,
...
export const getAppontmentsByDoctor = async (req, res)=>{
  try{
    ...
    const items = await Appointment.find(filter).sort({date : 1 , time : 1}).skip(skip).limit(limit).populate("doctorId", "name specialization imageUrl").lean();
    const total = await Appointment.countDocuments(filter);
    return res.json({
      success: true,
      appointments: items,
    ...
    console.error("getAppontmentsByDoctor unexpected:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
}
