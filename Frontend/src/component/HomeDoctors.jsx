...
import { Medal, MousePointer2Off, ChevronRight } from "lucide-react";
  ...
  return (
    <section className={h.section}>
      <div className={h.container}>
        ...
        {/*  refetch the api to get the response */}
        {loading ? (
          ...
          <div className={h.doctorsGrid}>
            {preview.map((doctor) => (
              <article key={doctor.id} className={h.article}>
                {doctor.available ? (
                  <Link to={`/doctors/${doctor.id}`} state={{ doctor: doctor }}>
                ...
                <div className={h.cardBody}>
                  <h3 className={h.doctorName} id={`doctor-${doctor.id}-name`}>
                    {doctor.name}
                  </h3>
